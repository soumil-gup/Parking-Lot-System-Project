...
- **Separation of concerns**: the `ParkingLot` class owns all business logic
  and SQLite access. `main.cpp` only translates HTTP requests/responses and
  never touches the database directly — the same layering pattern used in
  most backend frameworks (controller vs. service/repository layer).
- **Billing logic**: fee is calculated per vehicle type, rounded up to the
  next hour, with a 1-hour minimum.
- **Slot allocation**: currently first-available (`ORDER BY id LIMIT 1`).
- **Persistence**: SQLite was chosen over flat files since it provides
  transactions and simple querying without the overhead of running a
  separate database server — appropriate for a single-instance app like this.

## Future Improvements

- Add authentication (admin login) before parking/releasing
- Add a "reserve a slot in advance" feature
- Switch slot allocation to nearest-slot / zone-based logic
- Add unit tests for `ParkingLot` fee calculation and edge cases
- Containerize with Docker, or deploy to a small VPS
